...
namespace Microsoft.Band.Tiles.Pages
{
  public sealed class PageLayout
  {
      ...
      get
      {
        if (this.Root != null)
        {
          ...
          while (queue.Count > 0)
          {
            ...
            if (element is PagePanel pagePanel)
            {
              foreach (PageElement element1 in (IEnumerable<PageElement>) pagePanel.Elements)
    ...
    private int GetElementCountAndGenerateMissingIDs()
    {
      ...
      short nextId = 1;
      int generateMissingIds = 0 + 1 + this.Root.GetElementCountAndIDs(existingIDs);
      this.Root.GenerateMissingIDs(existingIDs, ref nextId);
      return generateMissingIds;
    }
    ...
    internal static PageLayout DeserializeFromBand(ICargoReader reader)
    {
      ...
      PageElement root = (PageElement) null;
      ...
      if (num4 == 1)
      {
        root = PageElement.DeserializeFromBand(reader);
        if (!(root is PagePanel))
          throw new SerializationException(BandResources.BandTilePageTemplateUnexpectedElementType);
      }
      return new PageLayout(root as PagePanel);
    }
  }
}
