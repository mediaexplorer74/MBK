...
namespace MobileBandSync.Common
{
  public class SyncViewModel : INotifyPropertyChanged
  {
    ...
    public void OnPropertyChanged(string propertyName = null) => this.PropertyChanged((object) this, new PropertyChangedEventArgs(propertyName));
    ...
    public async Task<IEnumerable<WorkoutItem>> StartDeviceSync()
    {
      if (this.Connected && this.CurrentBand != null)
      {
        ...
        byte[] sensorLog = await this.CurrentBand.GetSensorLog(new Action<string>(this.Report), new Action<ulong, ulong>(this.Progress), this.CleanupSensorLog, this.StoreSensorLogLocally);
        if (sensorLog != null)
        {
          ...
          try
          {
            ...
            List<WorkoutItem> workouts = await WorkoutDataSource.ImportFromSensorlog(sensorLog, new Action<string>(this.Status), new Action<ulong, ulong>(this.Progress));
            if (this.CurrentBand != null)
            {
              if (workouts.Count > 0)
              {
                this.StatusText = this.ResourceLoader.GetString("Storing") + " " + (object) workouts.Count + " " + (workouts.Count == 1 ? (object) this.ResourceLoader.GetString("Workout") : (object) this.ResourceLoader.GetString("Workouts"));
                ulong stepLength = WorkoutDataSource.DataSource.SensorLogEngine.StepLength;
                List<long> longList = await WorkoutDataSource.StoreWorkouts(workouts, new Action<ulong, ulong>(this.Progress), stepLength);
    ...
    public async Task<IEnumerable<WorkoutItem>> StartSyncFromLogs()
    {
      ...
      if (folderAsync != null)
      {
        try
        {
          ...
          listWorkouts = await WorkoutDataSource.ImportFromSensorlog(folderAsync, new Action<string>(this.Status), new Action<ulong, ulong>(this.Progress));
        }
        catch (Exception ex)
        {
        }
    ...
    public async void Progress(ulong uiCompleted, ulong uiTotal)
    {
      ...
      else
      {
        ...
        await CoreApplication.MainView.CoreWindow.Dispatcher.RunAsync((CoreDispatcherPriority) 0, new DispatchedHandler((object) cDisplayClass400, __methodptr(\u003CProgress\u003Eb__0)));
      }
    }
