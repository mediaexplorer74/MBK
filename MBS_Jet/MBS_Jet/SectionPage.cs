...
namespace MobileBandSync
{
  public sealed class SectionPage : Page, IComponentConnector
  {
    ...
    public SectionPage()
    {
      ...
      WindowsRuntimeMarshal.AddEventHandler<TypedEventHandler<MapControl, object>>(new Func<TypedEventHandler<MapControl, object>, EventRegistrationToken>(workoutMap.add_LoadingStatusChanged), new Action<EventRegistrationToken>(workoutMap.remove_LoadingStatusChanged), new TypedEventHandler<MapControl, object>((object) this, __methodptr(Map_LoadingStatusChanged)));
    ...
    private async Task ShowWorkout(WorkoutItem workout)
    {
      ...
      try
      {
        ...
        ((UIElement) this.StatusGrid).put_Visibility((Visibility) 1);
    ...
    public async void CreateDistancePoint(WorkoutItem item, TrackItem trackpoint, int iDistance)
    {
      ...
      try
      {
        ...
        float num1 = (float) (DisplayInformation.GetForCurrentView().RawPixelsPerViewPixel * 2.0 / 6.0);
        int num2 = (int) ((iDistance > 0 ? 50.0 : 30.0) * (double) num1);
        string str = iDistance.ToString();
        CanvasRenderTarget distanceMarker = new CanvasRenderTarget((ICanvasResourceCreator) CanvasDevice.GetSharedDevice(), (float) num2, (float) num2, 96f);
        using (CanvasDrawingSession drawingSession = distanceMarker.CreateDrawingSession())
        {
          drawingSession.FillRectangle(0.0f, 0.0f, (float) num2, (float) num2, iDistance > 0 ? Colors.DarkRed : Colors.Green);
          drawingSession.DrawRectangle(2f * num1, 2f * num1, (float) num2 - 3f * num1, (float) num2 - 3f * num1, Colors.White, 5f * num1);
          if (iDistance > 0)
            drawingSession.DrawText(str, (float) ((str.Length > 1 ? 6.0 : 15.0) * (double) num1), (float) (1.0 * (double) num1), Colors.White, new CanvasTextFormat()
            {
        ...
        using (InMemoryRandomAccessStream stream = new InMemoryRandomAccessStream())
        {
          ...
          async.SetPixelData((BitmapPixelFormat) 87, (BitmapAlphaMode) 2, (uint) width, (uint) height, logicalDpi1, logicalDpi2, pixelBytes);
          await async.FlushAsync();
          mapIcon.put_Image((IRandomAccessStreamReference) RandomAccessStreamReference.CreateFromStream((IRandomAccessStream) stream));
          ...
          {
            Latitude = (double) (item.LatitudeStart + (trackpoint == null ? 0L : (long) trackpoint.LatDelta)) / 10000000.0,
            Longitude = (double) (item.LongitudeStart + (trackpoint == null ? 0L : (long) trackpoint.LongDelta)) / 10000000.0,
            Altitude = 0.0
          }));
    ...
    private async void Share_Tapped(object sender, TappedRoutedEventArgs e)
    {
      ...
      bool bResult = false;
      fileSavePicker.put_SuggestedStartLocation((PickerLocationId) 0);
      fileSavePicker.FileTypeChoices.Add("Garmin Training Center Database", (IList<string>) new List<string>()
      {
        ".tcx"
      });
    ...
    private async void Map_LoadingStatusChanged(MapControl sender, object args)
    {
      ...
      try
      {
        ...
        do
        {
          flag = await this.WorkoutMap.TrySetViewBoundsAsync(this.Viewport, new Thickness?(margin), (MapAnimationKind) 1);
          if (!flag)
        ...
        if (NetworkInterface.GetIsNetworkAvailable())
          this.WorkoutMap.put_Style((MapStyle) 3);
    ...
    private async Task AddTracks(WorkoutItem workout) => await ((DependencyObject) this).Dispatcher.RunAsync((CoreDispatcherPriority) 1, new DispatchedHandler((object) new SectionPage.\u003C\u003Ec__DisplayClass51_0()
    {
      \u003C\u003E4__this = this,
      workout = workout
    ...
    private async void WorkoutTracks_Loaded(object sender, TracksLoadedEventArgs e)
    {
      ...
      try
      {
        ...
        this.LoadChartContents(this.CurrentWorkout);
        // ISSUE: method pointer
        await ((DependencyObject) this).Dispatcher.RunAsync((CoreDispatcherPriority) 1, new DispatchedHandler((object) this, __methodptr(\u003CWorkoutTracks_Loaded\u003Eb__52_0)));
    ...
    private async Task LoadChartContents(WorkoutItem workout) => await ((DependencyObject) this).Dispatcher.RunAsync((CoreDispatcherPriority) 1, new DispatchedHandler((object) new SectionPage.\u003C\u003Ec__DisplayClass54_0()
    {
      \u003C\u003E4__this = this,
      workout = workout
    }, __methodptr(\u003CLoadChartContents\u003Eb__0)));

    private async void CleanupChart() => await ((DependencyObject) this).Dispatcher.RunAsync((CoreDispatcherPriority) 1, new DispatchedHandler((object) this, __methodptr(\u003CCleanupChart\u003Eb__55_0)));
    ...
    private async void Grid_Tapped(object sender, TappedRoutedEventArgs e)
    {
      ...
      if (!(((FrameworkElement) grid).FindName("heartLine") is LineSeries name))
        return;
      WorkoutItem dataContext = ((FrameworkElement) name).DataContext as WorkoutItem;
      ObservableCollection<DiagramData> itemsSource = name.ItemsSource as ObservableCollection<DiagramData>;
      ...
      if (position.X >= point2.X && position.X <= point1.X)
      {
        ...
        if (trackItem != null)
        {
          ...
          ((Shape) this.chartLine).put_Stroke((Brush) new SolidColorBrush(Colors.White));
          Line chartLine = this.chartLine;
          double x;
          this.chartLine.put_X2(x = position.X);
          double num2 = x;
          chartLine.put_X1(num2);
          this.chartLine.put_Y1(0.0);
          this.chartLine.put_Y2(((FrameworkElement) grid).ActualHeight);
          ...
          if (trackItem.SpeedMeterPerSecond <= 0.0)
            num5 = num6 = 0.0;
          this.StatusText.put_Text((trackItem.TotalMeters / 1000.0).ToString("0.000") + " km, " + trackItem.Elevation.ToString() + " m, " + num5.ToString() + ":" + num6.ToString("00") + "/km, " + num7.ToString("0.00") + " km/h, HR: " + trackItem.Heartrate.ToString() + ", GSR: " + trackItem.GSR.ToString() + ", Temp: " + trackItem.SkinTemp.ToString());
          ((UIElement) this.StatusGrid).put_Visibility((Visibility) 0);
          Geopoint point3 = new Geopoint(new BasicGeoposition()
          {
            Latitude = (double) (dataContext.LatitudeStart + (long) trackItem.LatDelta) / 10000000.0,
            Longitude = (double) (dataContext.LongitudeStart + (long) trackItem.LongDelta) / 10000000.0,
            Altitude = 0.0
          });
          MapIcon mapIcon = new MapIcon();
          mapIcon.put_Location(point3);
          mapIcon.put_NormalizedAnchorPoint(new Point(0.5, 1.0));
          ((MapElement) mapIcon).put_ZIndex(80);
          mapIcon.put_Image((IRandomAccessStreamReference) RandomAccessStreamReference.CreateFromUri(new Uri("ms-appx:///Assets/DetailPos.png")));
          mapIcon.put_Title("");
          ((MapElement) mapIcon).put_Visible(true);
          ...
          if (!this.IsGeopointInGeoboundingBox(this.GetBounds(this.WorkoutMap), point3))
            this.WorkoutMap.put_Center(point3);
          flag1 = true;
        }
      }
      if (flag1)
    ...
    public GeoboundingBox GetBounds(MapControl map)
    {
      ...
      double num1 = 156543.04 * Math.Cos(map.Center.Position.Latitude * 3.1415926535897931 / 180.0) / (111325.0 * Math.Pow(2.0, map.ZoomLevel));
    ...
    public bool IsGeopointInGeoboundingBox(GeoboundingBox bounds, Geopoint point) => point.Position.Latitude < bounds.NorthwestCorner.Latitude && point.Position.Longitude > bounds.NorthwestCorner.Longitude && point.Position.Latitude > bounds.SoutheastCorner.Latitude && point.Position.Longitude < bounds.SoutheastCorner.Longitude;
