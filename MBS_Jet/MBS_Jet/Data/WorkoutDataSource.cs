...
namespace MobileBandSync.Data
{
  public sealed class WorkoutDataSource
  {
    ...
    public static string GetMapServiceToken() => WorkoutDataSource._workoutDataSource.MapServiceToken;

    public static ObservableCollection<WorkoutItem> GetWorkouts() => WorkoutDataSource._workoutDataSource.Workouts;

    public static void SetMapServiceToken(string strServiceToken) => WorkoutDataSource._workoutDataSource.MapServiceToken = strServiceToken;
    ...
    public static async Task<string> GetWorkoutSummaryAsync() => WorkoutDataSource._workoutDataSource.Summary;

    public static ObservableCollection<WorkoutItem> WorkoutList => WorkoutDataSource._workoutDataSource.Workouts;
      ...
      Action<ulong, ulong> Progress)
    {
      ...
      List<Workout> Workouts = await WorkoutDataSource._workoutDataSource.ReadWorkoutsFromSensorLogs(sensorLogFolder);
      ...
      return await WorkoutDataSource._workoutDataSource.AddWorkouts(Workouts, Status: Status, Progress: Progress, ulStepLength: ulStepLength);
    }
      ...
      Action<ulong, ulong> Progress)
    {
      if (!WorkoutDataSource.DbInitialized)
        WorkoutDataSource.DbInitialized = await WorkoutDataSource._workoutDataSource.InitDatabase();
      WorkoutDataSource._workoutDataSource.SensorLogEngine.Sequences.Clear();
      List<Workout> Workouts = await WorkoutDataSource._workoutDataSource.ReadWorkoutsFromSensorLogBuffer(btSensorLog);
      if (Workouts == null || Workouts.Count <= 0)
        return new List<WorkoutItem>();
      int num = 0;
      foreach (Workout workout in Workouts)
    ...
    {
      this.WorkoutsFolder = await KnownFolders.DocumentsLibrary.CreateFolderAsync("Workouts", (CreationCollisionOption) 3);
      this.WorkoutDbFolder = await KnownFolders.DocumentsLibrary.CreateFolderAsync("WorkoutDB", (CreationCollisionOption) 3);
      ...
      try
      {
        ...
        UICommand yesCommand = new UICommand("Yes", WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9__59_0 ?? (WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9__59_0 = new UICommandInvokedHandler((object) WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9, __methodptr(\u003CInitDatabase\u003Eb__59_0))));
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
        // ISSUE: reference to a compiler-generated field
        // ISSUE: method pointer
        UICommand uiCommand = new UICommand("No", WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9__59_1 ?? (WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9__59_1 = new UICommandInvokedHandler((object) WorkoutDataSource.\u003C\u003Ec.\u003C\u003E9, __methodptr(\u003CInitDatabase\u003Eb__59_1))));
            ...
            try
            {
              ...
              if (databaseFile != null)
              {
                ...
                if ((await databaseFile.GetBasicPropertiesAsync()).DateModified > oldProp.DateModified)
                {
                  ...
                  {
                    await oldDb.DeleteAsync();
                    StorageFile storageFile = await databaseFile.CopyAsync((IStorageFolder) this.DatabaseFolder);
                  }
                }
      ...
      using (SqliteConnection db = new SqliteConnection(string.Format("Filename={0}", (object) Path.Combine(this.DatabaseFolder.Path, "workouts.db"))))
      {
        ...
        {
          await db.OpenAsync();
          SqliteDataReader sqliteDataReader1 = await new SqliteCommand("CREATE TABLE IF NOT EXISTS Tracks (TrackId INTEGER PRIMARY KEY AUTOINCREMENT, WorkoutId INTEGER NOT NULL, SecFromStart INT, LongDelta INT, LatDelta INT, Elevation INT, Heartrate TINYINT, Barometer INT, Cadence TINYINT, SkinTemp TINYINT, GSR INT, UVExposure INT )", db).ExecuteReaderAsync();
          SqliteDataReader sqliteDataReader2 = await new SqliteCommand("CREATE TABLE IF NOT EXISTS Sleep (SleepId INTEGER PRIMARY KEY AUTOINCREMENT, SleepActivityId INTEGER NOT NULL, SecFromStart INT, SegmentType TINYINT, SleepType TINYINT, Heartrate TINYINT )", db).ExecuteReaderAsync();
          SqliteDataReader sqliteDataReader3 = await new SqliteCommand("CREATE TABLE IF NOT EXISTS Workouts (WorkoutId INTEGER PRIMARY KEY AUTOINCREMENT, WorkoutType TINYINT, Title NVARCHAR(128) NULL, Notes NVARCHAR(2048) NULL, Start DATETIME, End DATETIME, AvgHR TINYINT, MaxHR TINYINT, Calories INT, AvgSpeed INT, MaxSpeed INT, DurationSec INT, LongitudeStart INT8, LatitudeStart INT8, DistanceMeters INT8, LongDeltaRectSW INT, LatDeltaRectSW INT, LongDeltaRectNE INT, LatDeltaRectNE INT )", db).ExecuteReaderAsync();
        }
        catch (Exception ex)
        {
    ...
    public static async Task<bool> BackupDatabase(StorageFolder targetFolder = null)
    {
      ...
      if (database != null)
      {
        if (targetFolder == null)
          targetFolder = await KnownFolders.DocumentsLibrary.CreateFolderAsync("WorkoutDB", (CreationCollisionOption) 3);
        bResult = await database.CopyAsync((IStorageFolder) targetFolder, database.Name, (NameCollisionOption) 1) != null;
      }
      return bResult;
    }
      ...
      byte[] btSensorLog)
    {
      ...
      try
      {
        if (this.SensorLogEngine.Sequences.Count > 0)
          return await this.SensorLogEngine.CreateWorkouts(ExportType.HeartRate | ExportType.Cadence | ExportType.Temperature | ExportType.GalvanicSkinResponse);
      }
      catch (Exception ex)
      {
      }
      ...
      StorageFolder SensorLogFolder)
    {
      ...
      try
      {
        ...
        foreach (StorageFile file in (IEnumerable<StorageFile>) await SensorLogFolder.GetFilesAsync())
        {
          ...
          dictFiles.Add(new DateTime(int.Parse(str.Substring(str.Length - 24, 2)), month, day, hour, minute, second), str);
        }
        foreach (DateTime key in (IEnumerable<DateTime>) dictFiles.Keys.OrderBy<DateTime, DateTime>((Func<DateTime, DateTime>) (d => d)))
        {
          using (Stream fileStream = await ((IStorageFolder) SensorLogFolder).OpenStreamForReadAsync(dictFiles[key]))
          {
      ...
      ulong ulStepLength = 0)
    {
      ...
      ExportType type = ExportType.HeartRate | ExportType.Cadence | ExportType.Temperature | ExportType.GalvanicSkinResponse;
      try
      {
            ...
            else
            {
              if (workout.Summary != null)
              {
                ...
                else if (workout.Summary.HFAverage <= 120)
                {
                  ...
                  str = workout.Summary.HFAverage >= 140 ? (workout.Summary.HFAverage >= 145 ? (workout.Summary.HFAverage >= 151 ? (workout.Summary.HFAverage >= 160 ? resourceLoader.GetString("WorkoutMaximum") : resourceLoader.GetString("WorkoutHard")) : resourceLoader.GetString("WorkoutModerate")) : resourceLoader.GetString("WorkoutLight")) : resourceLoader.GetString("WorkoutWarmup");
                double num3 = 1000.0 / (workout.Summary.Distance / workout.Summary.Duration) / 60.0;
                ...
                workoutData.Title = workout.StartTime.ToString((IFormatProvider) WorkoutDataSource.AppCultureInfo) + " " + str + " " + ((double) workoutData.DistanceMeters / 1000.0).ToString("F2", (IFormatProvider) WorkoutDataSource.AppCultureInfo) + " km " + num6.ToString("F2", (IFormatProvider) WorkoutDataSource.AppCultureInfo) + " min/km " + workoutData.AvgHR.ToString("F0") + " bpm";
                if (workoutData.Notes == null || workoutData.Notes.Length == 0)
                  workoutData.Notes = "Sensor log import " + DateTime.Now.ToString((IFormatProvider) WorkoutDataSource.AppCultureInfo);
              }
              else
                workoutData.Title = workout.StartTime.ToString((IFormatProvider) WorkoutDataSource.AppCultureInfo) + " Workout";
              foreach (WorkoutPoint trackPoint in workout.TrackPoints)
              {
                ...
                if (workoutData.LongitudeStart == 0L)
                {
                  ...
                  trackItem.LongDelta = (int) (trackPoint.Position.LongitudeDegrees * 10000000.0 - (double) workoutData.LongitudeStart);
                if (workoutData.LatitudeStart == 0L)
                {
                  ...
                  trackItem.LatDelta = (int) (trackPoint.Position.LatitudeDegrees * 10000000.0 - (double) workoutData.LatitudeStart);
    ...
    public static async Task<WorkoutItem> GetWorkoutAsync(int workoutId)
    {
      ...
      IEnumerable<WorkoutItem> source = WorkoutDataSource._workoutDataSource.Workouts.Where<WorkoutItem>((Func<WorkoutItem, bool>) (workout => workout.WorkoutId == workoutId));
    ...
    {
      await WorkoutDataSource._workoutDataSource.GetWorkoutDataAsync();
      IEnumerable<TrackItem> source = WorkoutDataSource._workoutDataSource.Workouts.SelectMany<WorkoutItem, TrackItem>((Func<WorkoutItem, IEnumerable<TrackItem>>) (workout => (IEnumerable<TrackItem>) workout.Items)).Where<TrackItem>((Func<TrackItem, bool>) (item => item.UniqueId.Equals(uniqueId)));
