...
namespace MobileBandSync.Data
{
  public class WorkoutItem
  {
    ...
    public Visibility DownVisibility
    {
      get
      {
        if (this.Parent != null && this.Parent.Count > 0)
        {
          ...
          {
            if (this.WorkoutType == (byte) 21 && this.Parent[index - 1].WorkoutType == (byte) 21 || this.WorkoutType != (byte) 21 && this.Parent[index - 1].WorkoutType != (byte) 21)
              return (Visibility) 0;
          }
        }
        return (Visibility) 1;
    ...
    public event EventHandler<TracksLoadedEventArgs> TracksLoaded
    {
      ...
      remove => EventRegistrationTokenTable<EventHandler<TracksLoadedEventArgs>>.GetOrCreateEventRegistrationTokenTable(ref this.m_currentWorkout).RemoveEventHandler(value);
    }
    ...
    public async Task CopyToExternal(string tcxFile)
    {
      try
      {
        ...
        StorageFolder targetPath = await StorageFolder.GetFolderFromPathAsync(tcxFile.Remove(tcxFile.LastIndexOf('\\')));
        StorageFile storageFile = await (await ApplicationData.Current.LocalFolder.GetFileAsync(targetFile)).CopyAsync((IStorageFolder) targetPath, targetFile, (NameCollisionOption) 1);
    ...
    public async Task<bool> ExportWorkout(StorageFile tcxFile)
    {
      ...
      if (tcxFile != null)
      {
        try
        {
          StorageFile createFile = await ApplicationData.Current.LocalFolder.CreateFileAsync(tcxFile.Name, (CreationCollisionOption) 1);
